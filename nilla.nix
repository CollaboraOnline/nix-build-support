...
in nilla.create ({ config }: {
  config = let
    ...
    abi = "arm64-v8a";
    mkAndroidSdk = { androidenv, ndkVersion, cmakeVersion ? "latest", buildToolsVersion ? "latest", includeSources ? false, platformToolsVersion ? "latest" }: androidenv.composeAndroidPackages {
  ...
  in {
    ...
    shells.core = {
      ...
      shell = { lib, mkShell, ccache, libreoffice-collabora, boost, bsh, ... }:
        mkShell {
          packages = [
            ccache
          ];
    ...
    packages.android-poco = {
      ...
      package = { stdenv, fetchFromGitHub, androidenv, nettools }: let # nettools is for 'hostname'... weird but ok whatever
        version = "1.14.2";
    ...
    packages.android-zstd = {
      ...
      package = { stdenv, fetchFromGitHub, androidenv, cmake }: let
        androidSdk = (mkAndroidSdk {
          inherit androidenv;
    ...
    shells.online = {
      ...
      # Define our shell environment.
      shell = { lib, mkShell, ccache, collabora-online, typescript-language-server, ... }:
        mkShell {
          packages = [
            ccache
    ...
    shells.android-online = {
      ...
      # Define our shell environment.
      shell = { lib, mkShell, ccache, collabora-online, android-studio, androidenv, jdk17, libgcc, jdt-language-server, typescript-language-server, chromium, ... }: let
        androidSdk = (mkAndroidSdk {
          inherit androidenv;
